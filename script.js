...
function renderBeer(data) {
    ...
    const styleHtml = `<span onclick="toggleStyleInfo('${styleName}')" 
                          style="color:var(--accent); text-decoration:underline; cursor:pointer; font-weight:bold;">
                          ${styleName} ⓘ
                       </span>`;
...
function toggleStyleInfo(styleName) {
    const card = document.getElementById('styleDetailsCard');
    
    // Close if already open
    if (card.style.display === 'block' && document.getElementById('sdName').textContent === styleName) {
